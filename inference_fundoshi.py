...
# API TOGGLE
# =========================================================
ENABLE_API     = True                                       # ← True = kirim ke ORDS, False = display only
ORDS_URL       = "http://172.16.173.211/ords/intranet/painting/ocr_fundoshi"
TIMEOUT_SEC    = 10
SEND_COOLDOWN_SEC = 2.0   # minimal jeda antar pengiriman
...
def run_ocr(ocr,
    ...
    Hasil + confidence ditulis ke result_dict.
    """
    global ocr_running
    try:
        # --- Frame No ---
...
def main():
    ...
    while True:
        ...
        if r.boxes is not None and len(r.boxes) > 0:
            ...
            # ── SESSION MANAGEMENT ────────────────────────
            candidate_vin = result_dict["frame"]

            if candidate_vin and candidate_vin != current_vin_session:
                print(f"[SESSION] New VIN detected: {candidate_vin} — reset all fields")
                current_vin_session = candidate_vin

                # reset history field lain
                carrier_hist.clear()
            ...
            if all_ready:
                ...
                if (is_cooldown_ok
                    ...
                    if ENABLE_API:
                        ...
                        # API disabled — cukup log ke terminal
                        session_sent = True
            ...
            sent_status = "SENT ✓" if session_sent else f"STABLE: {stable_frame_counter}/{STABLE_REQUIRED}"
